...
import random
<<<<<<< HEAD
from models.nerfplusplus.helper import sample_rays_in_bbox, sample_rays_in_bbox_list
import cv2
import pickle
...
<<<<<<< HEAD
    ...
    def read_data(self, instance_dir, image_id, out_instance_seg = False, contract = True, smoothing_loss=False, out_src_view=False):
        ...
        if self.split == 'train':
            all_c2w, _, focal, img_size, self.RTs, _ = read_poses(pose_dir_train, img_files_train, output_boxes=True, contract = contract)
            img_files = img_files_train[:100]
            base_dir = base_dir_train
        elif self.split == 'val':
            all_c2w_train, all_c2w_val, focal, img_size, self.RTs, _ = read_poses(pose_dir_train, img_files_train, output_boxes=True, contract = contract)
            all_c2w =  np.concatenate((all_c2w_train, all_c2w_val), axis=0)
            img_files = img_files_train
            base_dir = base_dir_train
        else:
            _, _, focal, img_size, self.RTs, poses_scale_factor = read_poses(pose_dir_train, img_files_train, output_boxes=True, contract = contract)
            all_c2w_val = read_poses_val(pose_dir_test, img_files_test, poses_scale_factor)
            img_files = img_files_test
            base_dir = base_dir_test

            print("all_c2w", all_c2w_val.shape)
            ref_pose = all_c2w_val[50]
            all_c2w_test = []
            for i in range(40):
                all_c2w_test.append(move_camera_pose(np.copy(ref_pose), i/40))
            all_c2w = np.array(all_c2w_test)
            print("all_c2w new", all_c2w.shape)
...
<<<<<<< HEAD
        ...
        if out_instance_seg:
            if smoothing_loss:
                rays_o_near, _, rays_d_near, _ = get_rays(directions, c2w_near, output_view_dirs=True, output_radii=True)
                rays_o = torch.cat((rays_o, rays_o_near), 0)
                rays_d = torch.cat((rays_d, rays_d_near), 0)
                return rays_o, view_dirs, rays_d, img, instance_mask, inst_seg, nocs_2d, radii, pose, torch.tensor(focal, dtype=torch.float32), torch.tensor(c, dtype=torch.float32)
            else:
                if out_src_view:
                    return rays_o, view_dirs, rays_d, img, instance_mask, inst_seg, nocs_2d, radii, pose, torch.tensor(focal, dtype=torch.float32), torch.tensor(c, dtype=torch.float32), src_views_num
                else:
                    return rays_o, view_dirs, rays_d, img, instance_mask, inst_seg, nocs_2d, radii, pose, torch.tensor(focal, dtype=torch.float32), torch.tensor(c, dtype=torch.float32)
        else:
            if smoothing_loss:
                ...
                if out_src_view:
                    return rays_o, view_dirs, rays_d, img, instance_mask, nocs_2d, radii, pose, torch.tensor(focal, dtype=torch.float32), torch.tensor(c, dtype=torch.float32), src_views_num
                else:
                    return rays_o, view_dirs, rays_d, img, instance_mask, nocs_2d, radii, pose, torch.tensor(focal, dtype=torch.float32), torch.tensor(c, dtype=torch.float32)
=======
        directions = get_ray_directions(h, w, focal) # (h, w, 3)
...
<<<<<<< HEAD
            ...
            for train_image_id in range(0, NV):
                if train_image_id not in src_views_num:
                    continue
                _, _, _, img, _, _, _, c2w, f, c =  self.read_data(instance_dir, train_image_id)
...
<<<<<<< HEAD
            ...
            #Load desitnation view data
                cam_rays, cam_view_dirs, cam_rays_d, img_gt, instance_mask, nocs_2d, camera_radii, _, _, _ =  self.read_data(instance_dir, dest_view_num)
                ...
                if patch:
                    ...
                    rgbs = rgbs.view(height, width, 3)[x:x+30, y:y+30, :].reshape(-1,3)
                    nocs_2ds = nocs_2ds.view(height, width, 3)[x:x+30, y:y+30, :].reshape(-1,3)
                    masks = masks.view(height, width)[x:x+30, y:y+30].reshape(-1,1)
                    radii = radii.view(height, width)[x:x+30, y:y+30].reshape(-1,1)
                    rays = rays.view(height, width, 3)[x:x+30, y:y+30, :].reshape(-1,3)
                    rays_d = rays_d.view(height, width, 3)[x:x+30, y:y+30, :].reshape(-1,3)
                    view_dirs = view_dirs.view(height, width, 3)[x:x+30, y:y+30, :].reshape(-1,3)
                else:
...
<<<<<<< HEAD
            ...
            #Load desitnation view data
            cam_rays, cam_view_dirs, cam_rays_d, img_gt, instance_mask, inst_seg, nocs_2d, camera_radii, _, _, _ =  self.read_data(instance_dir, dest_view_num, out_instance_seg=True)
            ...
            for train_image_id in range(0, NV):
                if train_image_id not in src_views_num:
                    continue
                _, _, _, img, _, _, _, _, c2w, f, c =  self.read_data(instance_dir, train_image_id, out_instance_seg=True)
                img = Image.fromarray(np.uint8(img))
                img = T.ToTensor()(img)
                imgs.append(self.img_transform(img))
=======
...
<<<<<<< HEAD
            ...
            near_obj, far_obj, _ = sample_rays_in_bbox_list(self.RTs, rays.numpy(), view_dirs.numpy())
