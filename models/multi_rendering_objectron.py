...
from models.nerf import InstanceConditionalNeRF as ObjectNeRF
...
def render_rays_multi(
    ...
    # if bckg_img is not None:
    #     z_bckg = torch.ones((rays_list[0].shape[0], 64), device=rays_list[0].device) 
    #     z_vals_list +=[z_bckg]
    ...
    for idx, rays in enumerate(rays_list):
        ...
        print("z_vals", z_vals)
        print("z_vals min max", torch.min(z_vals), torch.max(z_vals))
        z_vals_list += [z_vals]
    ...
    # if bckg_img is not None:
    #     bckg_img = bckg_img.to(rays.device)
    #     rgbs_list+=[bckg_img.unsqueeze(1).repeat(1, N_samples, 1)]
    #     sigmas_list +=[torch.ones_like(z_bckg, device=rays.device) * -1e5]
    #     obj_ids_list += [torch.ones_like(z_bckg, device=rays.device) * 2]
    print("len(rays_list)", len(rays_list))
    for i in range(len(rays_list)):
        ...
        print("sigmas min max", torch.min(sigmas), torch.max(sigmas))
        rgbs_list += [rgbs]
        sigmas_list += [sigmas]
