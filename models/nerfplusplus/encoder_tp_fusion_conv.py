...
from models.nerfplusplus.encoder_pn import SpatialEncoder
...
class GridEncoder(nn.Module):
    ...
    def forward(self, images, poses, focal, c):
        ...
        world_grid_x = world_grids.reshape(1, NV, self.grid_size[0],
                                                          self.grid_size[1],
                                                          self.grid_size[2], 3)[:, 0, ..., 0:1]
        world_grid_x = world_grid_x.repeat(3,1,1,1,1)


        world_grid_y = world_grids.reshape(1, NV, self.grid_size[0],
                                                          self.grid_size[1],
                                                          self.grid_size[2], 3)[:, 0, ..., 1:2]
        world_grid_y = world_grid_y.repeat(3,1,1,1,1)


        world_grid_z = world_grids.reshape(1, NV, self.grid_size[0],
                                                          self.grid_size[1],
                                                          self.grid_size[2], 3)[:, 0, ..., 2:3]
        world_grid_z = world_grid_z.repeat(3,1,1,1,1)
        ...
        print("self.pillar_aggregator_yz(latent_inp_x)", self.pillar_aggregator_yz(latent_inp_x).shape)
        ...
        print("weights_yz", weights_yz.shape)
