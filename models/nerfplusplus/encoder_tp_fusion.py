...
def get_c(samples, imgs, poses, focal, c, encoder_type = 'resnet', attn = False, viewdirs = None, use_same_stride = False):
    ...
    if attn:
        ...
        # colors = torch.cat((colors.permute(0,2,1), samples_enc) , dim=-1)
...
class GridEncoder(nn.Module):
    ...
    def forward(self, images, poses, focal, c):
        ...
        # # ADD TRANSFORMER INSTEAD OF NAIVE MEAN. NOTE THAT TRANSOFER RIGHT NOW IS VERY SLOW
        # ###====================================================\n\n\n
        # ###TRANSFORMER BLOC FOR MVFUSION----------------------\n\n\n

        # if self.use_transformer:
        #     latent = latent.view(3, -1, L)
        #     latent = self.transformer_encoder(latent)
        #     latent = latent.view(1,3,self.grid_size[0],
        #                             self.grid_size[1],
        #                             self.grid_size[2], L)
        # ###====================================================\n\n\n
        # ###TRANSFORMER BLOC FOR MVFUSION----------------------\n\n\n
