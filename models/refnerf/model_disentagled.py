...
class Voxels(nn.Module):
    ...
    def forward(self, positions: torch.Tensor) -> torch.Tensor:
        ...
        print("positions", positions, torch.max(positions), torch.min(positions))
        positions = positions.reshape(1, -1, 1, 1, 3)

        # ind_norm = (
        #     (positions - self.xyz_min.cuda())
...
class Generator3D(torch.nn.Module):
    def __init__(self):
        ...
        self.voxel_size = 128  # options.voxel_size
        padd = (1, 1, 1)
...
class RefNeRFMLP(nn.Module):
    ...
    def forward(self, samples, viewdirs=None, shape_latent= None):
        
        if self.model_type == 'geometry':
            num_samples, feat_dim = samples.shape[1:]
            samples = samples.reshape(1, -1, 1, 1, feat_dim)
            density = self.generator_3d(samples)
            density = self.density_activation(density)
            density = density.reshape(-1, num_samples, self.num_density_channels)
            # voxels_out = self.generator_3d(shape_latent)
            # samples = samples.reshape(1, -1, 1, 1, feat_dim)
            # density = F.grid_sample(voxels_out, samples,
            #             padding_mode="border", align_corners=False)
            # density = density.reshape(-1, num_samples, self.num_density_channels)
            assert not density.isnan().any()
            return dict(
                density=density
            )
        else:
...
class RefNeRF(nn.Module):
    def __init__(
        ...
        self.mlp = RefNeRFMLP(self.deg_view, model_type= self.model_type)

    def forward(self, rays, randomized, white_bkgd, near, far, embedding_instance=None):
        ret = []
        if self.model_type == 'geometry':
            self.ray_shape = 'linear'
            self.num_levels = 1
...
class LitRefNeRF(LitModel):
    def __init__(
        ...
        self.model = RefNeRF(model_type = self.hparams.model_type)
        #self.code_library = CodeLibrary(self.hparams)
    ...
    #     rays_o = self.train_dataset.rays_o
    #     rays_d = self.train_dataset.rays_d
    ...
    def training_step(self, batch, batch_idx):
        for k,v in batch.items():
            print(k,v.shape)
        if self.hparams.model_type =='geometry':
            # extra_info = dict()
            ...
            loss = loss(torch.clamp(opacity_coarse, 0, 1), instance_mask.float()).mean()
    ...
    def render_rays(self, batch, batch_idx, extra=None):
        if self.hparams.model_type =='geometry':
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.hparams.model_type=='geometry':
            # extra_info = dict()
            # extra_info.update(self.code_library(batch))
            # ret = self.render_rays(batch, batch_idx, extra_info)
    ...
    def configure_optimizers(self):
        ...
        return torch.optim.Adam(
            params=parameters, lr=self.lr_init, betas=(0.9, 0.999)
        )
...
if __name__ == '__main__':
    ...
    z = torch.randn(1,128)
    voxels = model(z)
    print(out['pred_voxel'].shape)
    positions = torch.randn((131072, 3))
    
    _, feat_dim = positions.shape
    positions = positions.reshape(1, -1, 1, 1, feat_dim)
    output = F.grid_sample(voxels, positions,
                            padding_mode="border", align_corners=False)
