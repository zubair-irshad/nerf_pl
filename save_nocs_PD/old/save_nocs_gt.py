...
def get_bbox_from_segmap(segmap):
...
def get_bbox_from_3d(RTs, camera_pose):
    bboxes = []
    names = []
...
for folder in folders:
    ...
    for i, img_file in enumerate(img_files):
        ...
        all_c2w, focal, img_size, RTs, raw_boxes = read_poses(pose_dir_train, img_files=img_files, output_boxes=True)
        ...
        bboxes_mask, ids = get_bbox_from_segmap(segmap)

        # transform 3D bboxes to 2D
        camera_pose = convert_pose(np.array(all_c2w[i]))
        bboxes_from_3d, names = get_bbox_from_3d(RTs, camera_pose)
